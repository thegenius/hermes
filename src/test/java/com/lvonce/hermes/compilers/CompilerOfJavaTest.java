...
@Test
public class CompilerOfJavaTest {

    public static final Logger logger = LoggerFactory.getLogger(CompilerOfJavaTest.class);
    ...
    public void testCompileFile() {
        try {
            ...
            Compiler javaCompiler = new CompilerOfJava();
            Class<?>[] classTypes = javaCompiler.compile(file);
    ...
    public void testCompileString() {
        ...
        try {
            Compiler javaCompiler = new CompilerOfJava();
            Class<?>[] classTypes = javaCompiler.compile("TestNew", createSourceString());
            Object obj = ReflectUtils.createInstance(classTypes[0]);
            int result = (int) ReflectUtils.invoke(obj, "add", 1, 2);
            assertEquals(result, 3);
        } catch (Exception e) {
